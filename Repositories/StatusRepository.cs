...
namespace KitchenApi.Repositories
{
    public class StatusRepository : IStatusRepository
    {
        ...
        {
            var statuses = await _context.Statuses.Find(s => true).ToListAsync();
            return statuses;
        }

        public async Task<Status> GetStatusAsync(string id, CancellationToken cancellationToken)
        {
            var status = await _context.Statuses.Find(s => s.Id.Equals(id)).SingleOrDefaultAsync();
            return status;
        }
        ...
        public async Task<bool> IsStatusExistAsync(string title, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation(isStatusExist ? $"status with title '{title}' is exist" :
